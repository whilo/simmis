...
(defn get-webhook-info [token]
    (http/get (str "https://api.telegram.org/bot" token "/getWebhookInfo")))
  ...
  ([mechanism [S peer [in out]]]
   (let [stop-fn (mechanism peer in)
         ...
         po (pub prev-out (fn [{:keys [type]}]
                            (or ({:is.simm.languages.chat/send-text ::send-text
                                  :is.simm.languages.chat/send-photo ::send-photo
                                  :is.simm.languages.chat/send-document ::send-document} type)
                                :unrelated)))
         send-text (chan)
     ...
     (go-loop-try S [{[chat-id msg] :args :as m} (<? S send-text)]
                  (when m
                    ...
                    (put? S next-in (assoc m
                                           :type :is.simm.languages.chat/send-text-reply
     ...
     (go-loop-try S [{[chat-id url] :args :as m} (<? S send-photo)]
                  (when m
                    ...
                    (put? S next-in (assoc m
                                           :type :is.simm.languages.chat/send-photo-reply
                                           :response (try
     ...
     (go-loop-try S [{[chat-id url] :args :as m} (<? S send-document)]
                  (when m
                    ...
                    (put? S next-in (assoc m
                                           :type :is.simm.languages.chat/send-document-reply
                                           :response (try
...
(comment
  ...
  (require '[is.simm.languages.chat :refer [send-text!]]
           '[is.simm.languages.bindings :refer [*chans*]])
